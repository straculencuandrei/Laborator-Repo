...
public class Main {
    ...
    public static void problemaUnu() {
        List<Integer> numere = new Random().ints(10, 5, 26).boxed().toList();
        ...
        List<Integer> interval = numere.stream().filter(n -> n >= 10 && n <= 20).toList();
        System.out.println("Elemente in intervalul 10-20: " + interval);

        List<Double> numereDouble = numere.stream().map(Integer::doubleValue).toList();
    ...
    public static void problemaDoi() {
        ...
        List<String> filtrate = cuvinte.stream().filter(c -> c.length() >= 5).toList();
        System.out.println("S-au gasit " + filtrate.size() + " cuvinte cu lungimea >= 5: " + filtrate);

        List<String> ordonate = filtrate.stream().sorted().toList();
